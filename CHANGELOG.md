...
- App Intents and Siri Shortcuts (F-2, issue #26): Full system integration with macOS 14+ `AppIntents` framework. Core intents for `NewNoteIntent` (creates `.md` note), `NewRecipeIntent` (creates `.cook` recipe note with `>>` metadata), `TakeNoteIntent` (quick capture into Inbox), `SearchNotesIntent` (matches titles, bodies, and parsed ingredients), `OpenNotesFolderIntent` (reveals vault in Finder), and `PublishSiteIntent` (runs static site export pipeline). Registered via `BANALShortcutsProvider` for Siri phrases ("New note in BANAL", "Take a note in BANAL", "New recipe in BANAL", "Search notes in BANAL", "Publish notes in BANAL").
