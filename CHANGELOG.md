...
## [Unreleased]
- Sidebar yields at narrow widths (issue #214): dragging the window below ~880pt wide now collapses the sidebar automatically so list + editor keep room; widen past ~920pt and it slides back. The toolbar sidebar toggle still works either way — if you re-open it at a small size, resizing won't fight you.
- Selection follows the list (issue #213): switching to an empty folder, the Published list with nothing published, or a search with no matches no longer leaves the editor pinned to the note you were just reading — it shows its empty state. Picking a folder with notes selects its first note instead of keeping the previous folder's note.
- Mixed vaults publish through the real Boris engine again (issue #210): a published recipe or Textile note alongside Markdown no longer fails ⇧⌘P with `EROUTEMISSING` / `LinkAuditFailed`. Oliver's finished HTML is staged as Markdown before the compile, so Boris emits recipe pages as its own outputs and its link audit resolves every route; the builtin path and `.publish/` shape are unchanged.
...
- Boris and Oliver ship inside the app (issue #209): `make app` and Xcode builds compile universal binaries from local checkouts or directly from GitHub `main` branches (`drawmeanelephant/boris` and `drawmeanelephant/oliver`) into `Contents/Helpers`, the locators prefer them, and Settings → Publish loses its binary pickers — recipe Read and Publish Site now work in the sandboxed app on a machine with nothing installed. Debug overrides stay available via `BANAL_OLIVER_BIN` / `BANAL_BORIS_BIN`; legacy paths in `.banal/config.json` still round-trip. AppleScript errors now surface their message instead of returning silently.
