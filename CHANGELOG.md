...
- System pickers for Contacts and Files (G-4, issue #35): Edit → Insert Contact… presents native `CNContactPicker` popover/sheet and inserts selected contact as standard Markdown `[Full Name](mailto:email)` or `[Full Name](tel:phone)`; Edit → Insert File… presents standard `NSOpenPanel`, copies the chosen file into the vault `assets/` directory (with unique collision numbering `-1`, `-2`), and inserts `[Original Filename](assets/filename)`. Text insertions preserve full single-step `⌘Z` undo history in TextKit.
- Drag note out to system (G-5, issue #36): Dragging a note row from the list out to Finder, Mail, Messages, or external editors provides the real file on disk via file URL (`public.file-url` / `kUTTypeFileURL`) and plain text (`public.utf8-plain-text`). Dropping on Finder creates an exact copy of the `.md`, `.cook`, or `.textile` file; dropping into Mail or Messages attaches the file or inserts note text; internal dragging onto sidebar folders continues to move notes cleanly within the vault.
- Images land in assets (G-1, issue #32): Dropping or pasting image files (PNG, JPEG, GIF, WebP, HEIC, SVG, TIFF) onto the editor copies raw image files directly to `<vault>/assets/`, avoids collisions with `-1`, `-2` suffixing (e.g. `diagram-1.png`), and inserts relative Markdown links `![](assets/<filename>)` at the drop point/caret with `⌘Z` undo support (`isRichText` stays `false`). Edit → Insert Photo… opens the native system image picker (`NSOpenPanel` restricted to supported image formats) and inserts the markdown tag. Managed via `AssetManager` in `BANALCore`.
- Import notes and folders as copy (G-3, issue #34): File → Import… (⌘I) presents an open panel to select multiple files or entire folder trees (Obsidian vaults, Bear exports, text archives). `VaultImporter` in `BANALCore` recursively copies note files (`.md`, `.textile`, `.cook`, and `.txt`/`.markdown` converted to `.md`) and associated assets into the active notes directory on disk, cleanly resolves filename collisions (e.g. `Note-2.md`), refreshes the note store, and selects the first imported note.
- Copy As formats for Markdown, Rich Text, and HTML (G-2, issue #33): Edit → Copy As submenu under Edit menu with Copy as Markdown (`⌥⇧⌘C`), Copy as Rich Text (`⌥⌘C`), and Copy as HTML. Seamlessly copies selected text snippet or full note body in the chosen format. Rich Text outputs formatted `public.rtf` in SF Pro typography with bold, italic, headings, lists, code fences, and links. HTML outputs clean semantic tags to `public.html` + `public.utf8-plain-text`. Standard `⌘C` plain text copying is untouched.
...
- System furniture & accessibility sit (F-7, issue #31): Comprehensive VoiceOver accessibility pass across sidebar, note list, editor, recipe read, prose read, settings panes, and vault pickers. Enhanced spoken semantic descriptions (folder nesting, item counts, recipe tags, published states, relative timestamps, and preview snippets); support for macOS Increase Contrast with sharp 1pt column and search separators; support for Reduce Motion suppressing sliding transition animations; testing brief and verification script documented in `docs/TESTING-SYSTEM.md`.
...
- Print, Share, and Services menu integration (F-5, issue #29): File → Print (`⌘P`) formats notes with clean SF Pro typography and standard macOS pagination via `NSPrintOperation` (formatted document in Edit mode, recipe layout with title, metadata, ingredient list, cookware, and numbered steps in Recipe Read mode). File → Share presents `NSSharingServicePicker` with note file URL and plain text payload. macOS Services menu ("New BANAL Note" / `newNoteService`) is registered in `Info.plist` and captures plain text selections from any application into `Inbox/` notes.
